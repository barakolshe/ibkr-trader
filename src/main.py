...
def live_trade_loop() -> None:
    ...
    while True:
        ...
        if now_date < now_date.replace(hour=10, minute=50, second=0):
            logger.info(
                f"Sleeping {(now_date.datetime - now_date.replace(hour=10, minute=45, second=0).datetime).seconds // 60} minutes"
            )
            if not sleep_until(now_date.replace(hour=10, minute=50, second=0)):
                return
            live_trade()
        if not sleep_until(
            now_date.shift(days=days_shift).replace(hour=10, minute=50, second=0)
        ):
            return
        live_trade()
